...
def rbf_kernel(row_data, col_data, sigma):
    ...
    #############################################
    return np.exp((-1/(2 * sigma**2)) * ((row_data.T.dot(row_data).diagonal() * np.ones((col_data.shape[1], 1))).T + np.ones((row_data.shape[1], 1)) * col_data.T.dot(col_data).diagonal() - 2 * row_data.T.dot(col_data)))
    #return np.exp((-1/(2 * sigma**2)) * (row_data.T.dot(row_data).diagonal() * np.ones((t.shape[1], 1)) + np.ones((t.shape[0], 1)) * col_data.T.dot(col_data).diagonal() - t))
